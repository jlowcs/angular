...
export {A};`
      ...
      renderer.addImports(output, [
        { name: '@angular/core', as: 'i0' },
        { name: '@angular/common', as: 'i1' }
      ]);
...
export {A};`
      ...
      decoratorsToRemove.set(analyzedClass.decorators[0].node.parent!, [analyzedClass.decorators[0].node]);
...
export {A};`
      ...
      const analyzedClass = analyzedFile.analyzedClasses[0];
      const decoratorsToRemove = new Map<ts.Node, ts.Node[]>();
      decoratorsToRemove.set(analyzedClass.decorators[0].node.parent!, [analyzedClass.decorators[1].node]);
      renderer.removeDecorators(output, decoratorsToRemove);
      expect(output.toString()).toEqual(`
/* A copyright notice */
import {Directive} from '@angular/core';
